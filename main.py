...
def get_ticker(coin1='btc', coin2='usd'):
    ...
    buy = response[f'{coin1}_{coin2}']['buy']
    sell = response[f'{coin1}_{coin2}']['sell']
...
def send_welcome(message):
    ...
    markup.add(btn1)
    text = 'Вы только что присоединились к tg боту, который выводит актуальную цену купли/продажи криптовалюты'
...
def answer(message):
    if message.text == '₿ BTC ₿':
        ...
        markup.add(btn1, btn2, btn3)
        markup.add(btn11)
        text = 'Я еще не знаю такие валюту/команду, лучше выбери из тех, которые я предлагаю!'
        bot.send_message(message.chat.id, text, reply_markup=markup)
